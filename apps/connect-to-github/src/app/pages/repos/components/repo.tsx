...
export function Repo({ name, state, repositoryName, labels }) {
  const label = state === RepoState.fetching ? 'Fetching Repo' : state === RepoState.runningReport ? 'Running' : 'Idle'
  return (
    <div className="relative pl-4 pr-6 py-5 hover:bg-gray-50 sm:py-6 sm:pl-6 lg:pl-8 xl:pl-6">
...

            {
              labels.map((label, index) => {
                return (
                  <Fragment>
                    <span>{ label }</span>
                    { (label.length - 1) !== index && (<span aria-hidden="true">&middot;</span>)}
                  </Fragment>
                )
              })
            }
